...
ROBUST  <- file.path(PROJECT, "code/robustness")
...
# Rural-only universe, matching code/build_control_group.R's (wind)
...
# Same single 2013 investment snapshot used by code/build_control_group.R
...
INVEST_2013 <- read_csv(file.path(PROJECT, "code/shared_data/invest_communes_2013.csv"),
...
load_panel <- function() {
  read_csv(file.path(PROJECT, "code/panel.csv"), col_types = col_spec) %>%
    mutate(dep = clean_dep(dep)) %>% filter(commune_fixe == 1)
}

load_turnover <- function() {
  read_csv(file.path(PROJECT, "code/shared_data/turnover_candidats.csv"), col_types = col_spec) %>%
...
# of treated propensities. Same formula as code/build_control_group.R
