...
PUB     <- file.path(PROJECT, "code/descriptive")
...
ctrl_matched  <- read_csv(file.path(PROJECT, "code/control_group_matched.csv"), col_types = col_spec) %>%
  pull(code_insee) %>% unique()
ctrl_baseline <- read_csv(file.path(PROJECT, "code/shared_data/control_group_baseline.csv"), col_types = col_spec) %>%
  pull(code_insee) %>% unique()
...
cat(sprintf("Saved -> %s\n", file.path(FIG, "cumulative_capacity_3sources.png")))
  ...
  cat("SKIPPED (not required for any of the 15 tables -- optional cross-source\n")
  cat("comparison figure; needs Parc_merged_multi_communes.csv and\n")
  cat("WindFarm_France_merged_projects.csv, not part of this package): ")
...
p_map <- ggplot() +
  ...
  labs(title = "Treatment and control communes",
       ...
       caption = paste(
         ...
         "Control pools as used in the TWFE/CS pipeline (code/build_control_group.R, code/shared_data/control_group_baseline.csv).",
         sep = "\n")) +
