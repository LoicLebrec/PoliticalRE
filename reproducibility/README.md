...
own panel to rural (density 5-7), never-treated communes — the same two
filters applied by `code/build_control_group.R` for the matched pool,
without that script's subsequent matching step. This does not
byte-for-byte reproduce the legacy file (no construction script for it
survives; two candidate explanations for the gap, commune fusion and the
matched pool's department-treatment-count filter, were tested and
rejected). It recovers 21,740 of the legacy file's 21,744 communes
(99.98%) and additionally selects 5,827 communes the legacy file did not
include. The effect on published results was checked directly by
rerunning the full pipeline under both pools before adopting this file as
canonical. Most affected numbers move little (0.12 percentage points or
less, identical N-treated, no sign changes). Two exceptions: in
`tab:main_results`, the Heckman Reelection coefficient loses significance
(-3.00\*, p<0.10, under the legacy pool -> -2.60, not significant, under
this file); in `tab:baseline_results`, the Callaway-Sant'Anna Abstention
coefficient gains significance (+0.35, not significant -> +0.49,
significant at 5%). Both were confirmed by the author before this file
was adopted; see `/data_prep/README.md` for the full account. The paper's
text should be checked against these two changes before submission. The
legacy file is preserved at
