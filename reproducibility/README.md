...
**2. `control_group_baseline.csv` (the unmatched control pool) cannot be
regenerated byte-for-byte.**
`code/shared_data/build_control_group_baseline.R` implements the
confirmed general method: filtering the project's own panel to rural
(density 5-7), never-treated communes — the same two filters applied by
`code/build_control_group.R` for the matched pool, without that script's
subsequent matching step. Applied to the current panel, this recovers
21,740 of the original file's 21,744 communes (99.98% agreement) and
additionally selects 5,827 communes not present in the original. Two
candidate explanations were tested and rejected: restricting to
`commune_fixe == 1` communes reduces rather than improves the match rate
(512 communes in the original file are themselves not
`commune_fixe == 1`), and applying the matched pool's "department with
more than 3 treated communes" filter reduces the match rate to 84.5%.
The residual filter has not been identified. The original frozen file
remains the authoritative input for the results reported in the paper;
this script is a documented approximation, not a replacement. This does
not affect use of the frozen file as distributed (it contains
no treated communes under the corrected treatment definition); it affects
only exact regeneration from source.
