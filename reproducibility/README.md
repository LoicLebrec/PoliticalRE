...
**1. `external_wind_voteshare.csv` is a frozen snapshot, no script rebuilds
it.** `wind_speed_100m` is probably from the Global Wind Atlas (see
`/data_prep`); `voix_gagnant_mean/min/max` was counted in-house from our
own election data, but the script that did the counting is gone. Either
way nothing here currently regenerates it -- it's frozen in
`code/external_wind_voteshare.csv` (99.6% coverage, keyed on `code_insee`
x `annee`) and `build_panel.R` merges it back in as its last step. Replace
this with a real fetch script if the wind-speed source ever gets pinned
down.

**2. `control_group_baseline.csv` (the unmatched control pool) can't be
regenerated byte-for-byte, but the method is known.** It's the project's
own panel filtered to rural (density 5-7), never-treated communes -- the
same two filters `code/build_control_group.R` uses for the matched pool,
just without that script's matching step. Checking this by hand against
the 2014 panel recovers 21,740 of its 21,744 communes (99.98%), which
confirms the method, but ~6,000 communes that rule lets through aren't
actually in the file -- so there's one more filter nobody's pinned down
yet. Doesn't affect using the file, no treated communes are in it either
way, just means it's not exactly reproducible from scratch.
...
which lives in the top-level `data/` directory, not in this repo (BPE is
open data, see `/data_prep/README.md`, but the two scripts that clean it
into this shape are missing). If those files aren't there, steps 13-14
will fail -- that's a known gap, not a bug in `run_pipeline.R`.
