...
pm_full        <- read_csv(file.path(PROJECT, "simple_regression/panel/panel_maires.csv"), col_types = col_sp) %>%
  mutate(dep = clean_dep(dep))
panel_raw_full <- read_csv(file.path(PROJECT, "simple_regression/panel/panel.csv"), col_types = col_sp) %>%
  mutate(dep = clean_dep(dep))
...
dette_raw <- read_csv(
  file.path(PROJECT, "simple_regression/CRcreu10/data/balancecompt/dette_communes.csv"),
  col_types = cols(code_insee = col_character(), .default = col_guess())) %>%
