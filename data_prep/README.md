...
the matched pool, without that script's subsequent matching step. This is
a decision, not a discovery: the exact original filter could not be
recovered (two candidate explanations, commune fusion and the matched
pool's department-treatment-count filter, were tested and rejected; the
rule above recovers 99.98% of the original file's 21,744 communes and
additionally selects 5,827 communes the original did not include).

The impact on published results was checked directly by rerunning the
full pipeline under both pools before adopting this file as canonical.
Most affected numbers move little (TWFE coefficients throughout, and most
of `tab:baseline_results`, change by 0.12 percentage points or less, with
identical N-treated and no sign changes). Two exceptions do not fit that
pattern and are disclosed here rather than only in a commit message: in
`tab:main_results`, the Heckman Reelection coefficient loses statistical
significance (-3.00\*, p<0.10, under the legacy pool; -2.60, not
significant, under this file); in `tab:baseline_results`, the
Callaway-Sant'Anna Abstention coefficient gains significance in the
opposite direction (+0.35, not significant, under the legacy pool; +0.49
(significant at 5%) under this file). Both changes were confirmed by the
author before this file was adopted as canonical; the paper's text should
be checked against these numbers before submission, since this file, not
the legacy one, now produces the pipeline's baseline-pool and Heckman
results. The original frozen file is preserved at
...
7), after confirming the change in the underlying pool does not
meaningfully affect published results.
