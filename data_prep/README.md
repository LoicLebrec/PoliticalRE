...
departments that already have 18-49 parks, for reasons not further
investigated — negligible at this scale, and not affected by which
threshold or park-status filter is tried).
...
-3.00 but still short of the p<0.10 threshold reported in the paper. This
one number should be checked against the paper's text before submission;
every other headline number, across all 15 tables, now matches the legacy
...
7). One number — the Heckman Reelection coefficient in `tab:main_results`
— moves close to but does not fully restore the paper's reported
significance under this file; see Section 7 for the exact values. This
should be checked against the paper's text before submission.
